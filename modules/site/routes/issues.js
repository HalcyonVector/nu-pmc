...
router.post('/:project_id', requireAuth, requireProjectScope(), upload.single('photo'), asyncHandler(async (req, res) => {
    ...
    // Poll: ✅ Acknowledged / ❌ Disputed
    if (body.assigned_vendor_id) {
      const notifSvc = require('../../../services/notifications');
      notifSvc.notifyVendor(
        body.assigned_vendor_id,
        `${issueNumber} — ${body.title?.slice(0, 80)} — please acknowledge.`,
        ['✅ Acknowledged', '❌ Disputed']
      ).catch(e => console.warn('[issues] vendor ack poll failed:', e.message));
    }
...
//
// v6.02: design_head sees only design-stream RFIs, services_head only services.
// PMC, Principal, site managers see all RFIs.
router.get('/rfi/:project_id', requireAuth, requireProjectScope(), asyncHandler(async (req, res) => {
    ...
    if (me.role === 'design_head' || me.role === 'services_head') {
      const expected = me.role === 'design_head' ? 'design' : 'services';
      filtered = rows.filter(r => !r.drawing_stream || r.drawing_stream === expected);
    }

    const Auth = require('../../auth/contract');
    const users = await Auth.functions.getUsers(filtered.flatMap(r => [r.raised_by, r.assigned_to].filter(Boolean)));
    filtered.forEach(r => {
...
router.post('/ncr/:project_id', requireAuth, requirePMC, asyncHandler(async (req, res) => {
    ...
    try {
      ...
      console.warn('[issues ncr] Principal endorsement poll failed:', e.message);
    }
