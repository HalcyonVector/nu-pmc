...
/**
 * Download a media object from the homeserver by mxc:// URI.
 * Used by services/matrix-media.js when the bot's reader job
 * encounters an inbound m.image / m.file / m.video / m.audio event
 * (delta brief §11.4).
 *
 * @param {string} mxcUrl    e.g. 'mxc://server.tld/abcdef'
 * @returns {Promise<Buffer>}
 */
async function downloadMedia(mxcUrl) {
  if (!mxcUrl) throw new MatrixError('mxcUrl required', { code: 'MISSING_MXC' });
  const m = String(mxcUrl).match(/^mxc:\/\/([^/]+)\/(.+)$/);
  if (!m) throw new MatrixError(`bad mxc URI: ${mxcUrl}`, { code: 'BAD_MXC' });
  const [, serverName, mediaId] = m;

  const mode = modeOf();
  if (mode !== 'LIVE') {
    // DRY_RUN / DISABLED — return an empty buffer so callers can still
    // exercise the persistence path in tests.
    return Buffer.alloc(0);
  }

  const url = `${_env().HOMESERVER}/_matrix/media/v3/download/${encodeURIComponent(serverName)}/${encodeURIComponent(mediaId)}`;
  const res = await http.get(url, {
    headers: { Authorization: `Bearer ${_env().BOT_TOKEN}` },
 ...
 *
 * Visual format per delta brief §10.2 (1 May 2026, tested on iPhone 16
 * Pro Max). The THICK rule (▬×14) bookends every message. NBSP final
 * line survives Matrix whitespace stripping so the bottom rule isn't
 * the last visible thing in the lockscreen preview.
 *
 * @param {string} emoji          e.g. '✍️', '⚠️', '💰' per delta brief §10.3
 ...
 */
const THICK = '▬▬▬▬▬▬▬▬▬▬▬▬▬▬';   // 14 chars — do not lengthen
const NBSP  = '\u00A0';             // non-breaking space — survives strip
...
function formatMessage(emoji, projectCode, description, actionType, actionPayload) {
  ...
  if (actionType === 'link') {
    return {
      msgtype: 'm.text',
      body:    `${opening}\n\n${actionPayload}${footer}`,
      format:  'org.matrix.custom.html',
      formatted_body:
        `<p>${THICK}</p>` +
        `<p><strong>${header}</strong></p>` +
        `<p><a href='${actionPayload}'>Open →</a></p>` +
        `<p>${THICK}</p>` +
        `<p>&nbsp;</p>`,
    };
  }
  ...
  return { msgtype: 'm.text', body: `${opening}${footer}` };
}

/**
 * Send a file (PDF, document) to a room. Call uploadMedia first to get mxcUrl.
 * Uses m.file msgtype. PDF and other non-image binary attachments use this.
 */
async function sendFile({ roomId, mxcUrl, filename = 'document.pdf', mimeType = 'application/pdf', body = '' }) {
  if (!roomId) throw new MatrixError('roomId required', { code: 'MISSING_ROOM' });
  if (!mxcUrl) throw new MatrixError('mxcUrl required (call uploadMedia first)', { code: 'MISSING_MXC' });
  ...
  if (mode === 'DRY_RUN') {
    return { outboxId: null, eventId: null, mode };
  }
  ...
  try {
    ...
    const eventId = res?.data?.event_id;
    return { outboxId: null, eventId, mode };
  } catch (err) {
    const msg = err.response?.data?.error || err.message;
    throw new MatrixError(`Matrix sendFile failed: ${msg}`);
  }
}

/**
 * Derive the Matrix user ID for an internal user from their username.
 * Pattern: @{username}:{homeserver_domain}
 * Domain extracted from MATRIX_BOT_USER_ID env var (same server).
 * Returns null if MATRIX_BOT_USER_ID is not set.
 */
function deriveMatrixUserId(username) {
  const botUser = process.env.MATRIX_BOT_USER_ID || '';
  const m = botUser.match(/^@[^:]+:(.+)$/);
  if (!m) return null;
  return `@${username.toLowerCase()}:${m[1]}`;
}

/**
 * Create a DM room between the bot and a user, returning the Matrix room ID.
 * Used to provision a personal digest room for new internal users.
 * Non-fatal — if Matrix is unreachable, returns null (room can be set later).
 */
async function createUserDMRoom(matrixUserId) {
  if (!matrixUserId) return null;
  const mode = modeOf();
  if (mode !== 'LIVE') return null;   // don't attempt in DRY_RUN / DISABLED

  try {
    const url = `${_env().HOMESERVER}/_matrix/client/v3/createRoom`;
    ...
    console.warn('[matrix-adapter] createUserDMRoom failed for', matrixUserId, '—', err.message);
    return null;
  }
}
