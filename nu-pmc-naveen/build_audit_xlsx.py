...
# Per-role rows — (role, schedule, issues_type, cns, approvals, reports, submittals, tasks_or_pay)
roles = [
    ('Principal (Naveen)',            'Yes', 'All open issues', 'Yes', 'Yes (all categories)',   '—', '—', '—'),
    ('Design Principal (Ajay)',       'Yes', 'All open issues', 'Yes', 'Yes (all categories)',   '—', '—', '—'),
    ('PMC Head (Murugesan, Praveen)', '—',   'All open issues', 'Yes', 'Yes (all categories)',   'Yes (daily reports)', '—', '—'),
    ('Design Head (Rajani)',          '—',   'Design stream',   '—',   'Drawings + Budget (design only)', '—', '—', '—'),
    ('Services Head (Shrinath)',      '—',   'Services stream', '—',   'Drawings + Budget (services only)', '—', '—', '—'),
    ('Finance Admin (Udupa)',         '—',   '—',               '—',   '—',                       '—', '—', 'Payments Queue'),
    ('Senior Site Manager (Praveen)', 'Yes', 'All open issues (project)', '—', 'Payments → GRN only', '—', '—', '—'),
    ('Site Manager (Arun)',           'Yes', 'All open issues (project)', '—', 'Payments → GRN only', '—', '—', '—'),
    ('Team Lead (Sahana)',            '—',   'My issues only',  '—',   '—',                       '—', 'Yes', '—'),
    ('Detailing Head',                '—',   'My issues only',  '—',   '—',                       '—', 'Yes', '—'),
    ('Jr Architect (Preethi, Satish)','—',   'My issues only',  '—',   '—',                       '—', '—', '—'),
    ('Services Engineer',             '—',   'My issues only',  '—',   '—',                       '—', '—', '—'),
    ('Coordinator (Pallavi)',         '—',   'My issues only',  '—',   '—',                       '—', '—', 'Today\'s Tasks'),
...
# Column widths
widths = [32, 10, 28, 8, 28, 22, 18, 26]
for i, w in enumerate(widths, 1):
    ws.column_dimensions[get_column_letter(i)].width = w

ws.row_dimensions[4].height = 42

# Audit notes block at bottom
row += 1
...
cats = [
    ('Drawings',
     ...
     '• Submittal reviews (review comments, status)\n'
     '• Change Notice approvals (peer below ₹1L, principal above)',
     'Principals, PMC Head, Design Head (design only), Services Head (services only), Audit'),

    ('Payments',
     '• GRN approvals (senior site below 5% budget, PMC any value, principal any)\n'
     '• Payment Request PMC-review (PMC Head)\n'
     '• Payment Request naveen-review (Principal)\n'
     '• Vendor engagement approvals (Principal)\n'
     '• Client claim approvals (Principal)',
     'Principals (all), PMC Head (GRN + PR), Senior Site / Site Manager (GRN only), Audit'),
    ...
    ('MOMs',
     ...
     '  (internal checkpoint — separate button then "issue to client")',
     'PMC Head (approves), Principals (visibility), Audit'),

    ('Other',
     '• Schedule change approvals routed via approvals queue\n'
     '• Weekly report approvals (rare)\n'
     '• Anything else that doesn\'t fit above 4',
     'Principals, PMC Head, Audit'),
]
...
for cat, inside, who in cats:
    ...
    c.fill = BG_SUBHEADER
    c.border = BOX
    c2 = ws2.cell(row=row, column=2, value=inside)
    c2.font = F_BODY
    c2.alignment = LEFT
    c2.border = BOX
    c3 = ws2.cell(row=row, column=3, value=who)
    c3.font = F_BODY
    c3.alignment = LEFT
    c3.border = BOX
...
# ════════════════════════════════════════════════
# SHEET 3 — Change log vs current state
# ════════════════════════════════════════════════
...
changes = [
    ...
    ('Issues count',
     'Single "8" number, same for all',
     'Role-filtered — stream-scoped for design/services heads, all for PMC/principals, assigned-only for team_lead/jr_architect'),
    ('CNs count',
     'Single "2" number, same for all',
     'Only visible to Principals + PMC Head + Audit (others don\'t approve CNs)'),
    ('Team count stat',
...
for area, cur, rev in changes:
    ...
    c.fill = BG_SUBHEADER
    c.border = BOX
    c2 = ws3.cell(row=row, column=2, value=cur)
    c2.font = F_BODY
    c2.alignment = LEFT
    c2.border = BOX
    c3 = ws3.cell(row=row, column=3, value=rev)
    c3.font = F_BODY
    c3.alignment = LEFT
    c3.border = BOX
...
# ════════════════════════════════════════════════
# SHEET 4 — Your approval / comments
# ════════════════════════════════════════════════
...
row = 5
for idx, (item, _, _) in enumerate(items, 1):
    c = ws4.cell(row=row, column=1, value=idx)
    c.font = F_BODY_BOLD
    c.alignment = CENTER
    c.fill = BG_SUBHEADER
    c.border = BOX
    c2 = ws4.cell(row=row, column=2, value=item)
    c2.font = F_BODY
    c2.alignment = LEFT
    c2.border = BOX
    c3 = ws4.cell(row=row, column=3, value='')
    c3.font = F_BODY
    c3.alignment = CENTER
    c3.border = BOX
    c3.fill = PatternFill('solid', start_color='FFFBEF')
    c4 = ws4.cell(row=row, column=4, value='')
    c4.font = F_BODY
    c4.alignment = LEFT
    c4.border = BOX
    c4.fill = PatternFill('solid', start_color='FFFBEF')
...
OUT = '/mnt/user-data/outputs/20260420 nu ProjectSummary Audit v1d.xlsx'
wb.save(OUT)
print(f'Saved: {OUT}')
