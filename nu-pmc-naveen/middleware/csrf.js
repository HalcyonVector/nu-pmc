...
function csrfMiddleware(req, res, next) {
  ...
  if (!sessionToken) {
    // No session token means the user has no logged-in session — but they
    // got past auth somehow (e.g. cookie session exists but token wasn't
    // issued, which can happen for sessions created before CSRF was enabled).
    // Treat as expired session, force re-login client-side.
