...
async function run() {
  ...
  for (const dv of stuckDrawings) {
    if (dv.days_pending >= 2) {
      ...
      // Notify Naveen + Ajay
      // WhatsApp notification via Twilio (Phase 2)
      console.log(`[overdue-checker] ESCALATE: Drawing ${dv.drawing_number} stuck ${dv.days_pending} days — notify principals`);
    }
  }
  ...
  for (const cn of stuckCNs) {
    console.log(`[overdue-checker] CN ${cn.cn_number} stuck ${cn.days_open} days — notify Naveen`);
    // WhatsApp to Naveen + Ajay: CN stuck, take over
  }
  ...
  // ── 6. PMC 7 AM daily digest — handled by sendPMC7AM() called from is7AM branch above
