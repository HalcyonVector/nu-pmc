...
router.post('/:project_id', requireAuth, requireProjectScope(), requireRole('site_manager','senior_site_manager','pmc_head','principal','design_principal'), upload.fields([
  ...
  try {
    ...
    try {
      ...
      // Trigger sign-off via the gate. Per signoff_workflows seed
      // (v5.31), grn_approval sequence is 'pmc' (single-approver poll
      // to the project's PMC head). The gate resolves the approver
      // from project_assignments, sends the poll to their Matrix DM,
      // and records a signoff_instance.
      //
      // Behavioural change vs pre-migration: the previous loop sent
      // to ALL users with role pmc_head (or senior_site_manager below
      // a value threshold). The brief specifies one approver only —
      // the project's assigned PMC head. The threshold-based fallback
      // to senior_site_manager is dropped per delta brief (sequence
      // is fixed at 'pmc' for grn_approval).
      try {
        ...
        await signoffGate.triggerSignoff(
          ...
          {
            question: `GRN ${num} — ${eng?.vendor_name || '—'} — ${validQty} ${body.unit || 'units'} — ₹${grnValue.toLocaleString('en-IN')} — approve?`,
            documentRow: { id: grnId, raised_by: req.session.user.id },
            triggeredBy:  req.session.user.id,
          }
        );
        if (deliveryNoteUrl) {
          // Delivery note URL is informational; the gate's poll
          // doesn't carry attachments. Log for now; future: post a
          // separate Matrix message in the same room with the URL
          // alongside the poll.
          console.log('[grn] delivery note URL', { grnId, url: deliveryNoteUrl });
        }
      } catch (err) {
        console.warn('[grn signoffGate.triggerSignoff]', err.message);
      }
    ...
    res.json({
      ...
      message: isUnplanned
        ? `GRN ${num} raised — flagged as unplanned delivery. Approval message sent.`
        : `GRN ${num} raised — approval message sent.`,
    });
