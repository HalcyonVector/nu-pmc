...
const formats = {
  icici: {
    ...
    buildRow: (idx, vendor, engagement, payment, projectCode, date) => {
      ...
      const debitAcc = process.env.ICICI_DEBIT_ACCOUNT || process.env.NU_BANK_ACCOUNT || '233705000984';
      return [
 ...
 */
async function generateBulkPaymentExcel(payments, projectCode, date) {
  ...
  payments.forEach((p, i) => {
    ...
    data.push(fmt.buildRow(i + 1, p.vendor, p.engagement, p.payment, projectCode, date));
  });
