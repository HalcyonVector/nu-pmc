...
describe('POST_COMPLETION_HOOKS', () => {
  ...
  test('final_settlement: hook is registered', () => {
    expect(Array.isArray(gate.POST_COMPLETION_HOOKS.final_settlement)).toBe(true);
    expect(gate.POST_COMPLETION_HOOKS.final_settlement.length).toBeGreaterThan(0);
  });

  test('final_settlement: only fires when result=approved', async () => {
    const hook = gate.POST_COMPLETION_HOOKS.final_settlement[0];
    db.query.mockClear();
    await hook({ id: 1, document_id: 7, project_id: 3, result: 'rejected',
                 workflow_type: 'final_settlement' });
    expect(db.query).not.toHaveBeenCalled();
  });

  test('final_settlement: no-op when no closures are flagged', async () => {
    const hook = gate.POST_COMPLETION_HOOKS.final_settlement[0];
    db.query.mockReset();
    db.query.mockResolvedValueOnce([[]]);   // empty closures list
    await hook({ id: 1, document_id: 7, project_id: 3, result: 'approved',
                 workflow_type: 'final_settlement' });
    // Only the SELECT — no UPDATE, no triggerSignoff cascade.
    expect(db.query).toHaveBeenCalledTimes(1);
    expect(db.query.mock.calls[0][0]).toMatch(/finance_step_blocked = 1/);
  });

  test('final_settlement: clears flag and re-triggers each blocked closure', async () => {
    const hook = gate.POST_COMPLETION_HOOKS.final_settlement[0];
    db.query.mockReset();
    adapter.sendPoll.mockReset();
    adapter.sendPoll.mockResolvedValue({ eventId: '$resume:s' });

    // The hook calls triggerSignoff under the hood; that fires more
    // queries we don't care about here. Rather than mocking the full
    // chain, return permissive defaults for everything past the first
    // two queries the hook itself owns. The contract we're checking:
    //   1. SELECT on project_closures with finance_step_blocked = 1
    //   2. UPDATE clearing the flag
    //   3. triggerSignoff fires (validated separately by emergency-CN test)
    db.query.mockImplementation((sql) => {
      if (/SELECT id, project_id, closure_block_id\s+FROM project_closures/.test(sql)) {
        return Promise.resolve([[{ id: 42, project_id: 3, closure_block_id: null }]]);
      }
      if (/UPDATE project_closures/.test(sql) && /finance_step_blocked = 0/.test(sql)) {
        return Promise.resolve([{ affectedRows: 1 }]);
      }
      if (/SELECT \* FROM signoff_workflows/.test(sql)) {
        return Promise.resolve([[{
          id: 50, workflow_type: 'project_closure',
          sequence: 'site_manager,design_lead,finance,principal',
          closing_minutes: null, quorum_required: 0, signoff_type: 'poll',
        }]]);
      }
      if (/FROM signoff_sequence_rules/.test(sql)) return Promise.resolve([[]]);
      if (/UPDATE signoff_instances/.test(sql) && /SET status = 'cancelled'/.test(sql)) {
        return Promise.resolve([{ affectedRows: 0 }]);
      }
      if (/INSERT INTO signoff_instances/.test(sql)) return Promise.resolve([{ insertId: 300 }]);
      if (/UPDATE signoff_instances/.test(sql)) return Promise.resolve([{ affectedRows: 1 }]);
      // Resolver lookups (roles → users)
      if (/FROM users/.test(sql)) {
        return Promise.resolve([[{ id: 60, role: 'site_manager', matrix_room_id: '!sm:s' }]]);
      }
      return Promise.resolve([[]]);
    });
    ...
    const updateCall = db.query.mock.calls.find(c =>
      /UPDATE project_closures/.test(c[0]) && /finance_step_blocked = 0/.test(c[0])
    );
    expect(updateCall).toBeTruthy();
    expect(updateCall[1]).toEqual([42]);

    // The hook ran past the UPDATE without throwing. triggerSignoff
    // is invoked downstream (its own test coverage validates the poll
    // path); the trigger-failure-isolation test below confirms the
    // loop reaches triggerSignoff for each closure even when one fails.
  });

  test('final_settlement: trigger failure does not crash other closures', async () => {
    const hook = gate.POST_COMPLETION_HOOKS.final_settlement[0];
    db.query.mockReset();
    adapter.sendPoll.mockReset();
    adapter.sendPoll.mockResolvedValue({ eventId: '$resume:s' });

    let workflowQueryCount = 0;
    db.query.mockImplementation((sql) => {
      if (/SELECT id, project_id, closure_block_id\s+FROM project_closures/.test(sql)) {
        return Promise.resolve([[
          { id: 42, project_id: 3, closure_block_id: null },
          { id: 43, project_id: 3, closure_block_id: null },
        ]]);
      }
      if (/UPDATE project_closures/.test(sql) && /finance_step_blocked = 0/.test(sql)) {
        return Promise.resolve([{ affectedRows: 1 }]);
      ...
      if (/SELECT \* FROM signoff_workflows/.test(sql)) {
        // First triggerSignoff (closure 42) explodes; second succeeds.
        workflowQueryCount++;
        if (workflowQueryCount === 1) return Promise.reject(new Error('boom'));
        return Promise.resolve([[{
          id: 50, workflow_type: 'project_closure',
          sequence: 'site_manager,design_lead,finance,principal',
          closing_minutes: null, quorum_required: 0, signoff_type: 'poll',
      ...
      if (/INSERT INTO signoff_instances/.test(sql)) return Promise.resolve([{ insertId: 301 }]);
      if (/UPDATE signoff_instances/.test(sql)) return Promise.resolve([{ affectedRows: 1 }]);
      if (/FROM users/.test(sql)) {
        return Promise.resolve([[{ id: 60, role: 'site_manager', matrix_room_id: '!sm:s' }]]);
      }
    ...
    // Should not throw despite the first closure's failure.
    await hook({ id: 1, document_id: 7, project_id: 3, result: 'approved',
                 workflow_type: 'final_settlement' });

    // Both flag-clearing UPDATEs ran.
