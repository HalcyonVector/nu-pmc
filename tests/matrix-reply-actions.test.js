...
describe('processVotesForRoom', () => {
  test('returns zeroes when room has no events', async () => {
    ...
    expect(out).toMatchObject({ scanned: 0, dispatched: 0, ignored: 0 });
  ...
  test('dispatches a poll-response event matching a signoff_instance row', async () => {
    ...
    expect(out).toMatchObject({ scanned: 1, dispatched: 1, ignored: 0 });
  ...
  test('ignores poll responses with no matching pending row', async () => {
    ...
    const out = await replyActions.processVotesForRoom(db, '!r:s');
    expect(out).toMatchObject({ scanned: 1, dispatched: 0, ignored: 1 });
  });
  ...
  // ── media wire-in (delta brief §11) ─────────────────────────────
  // The reader scans the same fetched events for m.image/m.file/
  // m.video/m.audio and dispatches each to matrix-media.processIncomingFile.

  test('media events trigger processIncomingFile and increment counters', async () => {
    const matrixMedia = require('../services/matrix-media');
    const spy = jest.spyOn(matrixMedia, 'processIncomingFile');

    const imageEv = {
      type: 'm.room.message',
      sender: '@a:s', event_id: '$img:s', origin_server_ts: 3000,
      content: { msgtype: 'm.image', body: 'photo.jpg', url: 'mxc://s/abc' },
    };
    const fileEv = {
      type: 'm.room.message',
      sender: '@a:s', event_id: '$file:s', origin_server_ts: 3001,
      content: { msgtype: 'm.file',  body: 'doc.pdf',   url: 'mxc://s/def' },
    };

    matrixAdapter.readMessages.mockResolvedValueOnce([imageEv, fileEv]);
    db.query
      .mockResolvedValueOnce([[{ last_seen_ts: 0 }]])     // cursor read
      .mockResolvedValueOnce([{ affectedRows: 1 }]);       // cursor update

    spy
      .mockResolvedValueOnce({ action: 'inserted',  id: 1 })
      .mockResolvedValueOnce({ action: 'duplicate', id: 2 });

    const out = await replyActions.processVotesForRoom(db, '!r:s');

    expect(spy).toHaveBeenCalledTimes(2);
    expect(spy.mock.calls[0][1]).toBe('!r:s');
    expect(spy.mock.calls[1][1]).toBe('!r:s');
    expect(out.mediaInserted).toBe(1);
    expect(out.mediaDuplicate).toBe(1);
    expect(out.mediaRejected).toBe(0);

    spy.mockRestore();
  });

  test('media intake errors do not break the per-room cycle', async () => {
    const matrixMedia = require('../services/matrix-media');
    const spy = jest.spyOn(matrixMedia, 'processIncomingFile');

    const ev1 = {
      type: 'm.room.message',
      sender: '@a:s', event_id: '$img1:s', origin_server_ts: 4000,
      content: { msgtype: 'm.image', body: 'a.jpg', url: 'mxc://s/1' },
    };
    const ev2 = {
      type: 'm.room.message',
      sender: '@a:s', event_id: '$img2:s', origin_server_ts: 4001,
      content: { msgtype: 'm.image', body: 'b.jpg', url: 'mxc://s/2' },
    };

    matrixAdapter.readMessages.mockResolvedValueOnce([ev1, ev2]);
    db.query
      .mockResolvedValueOnce([[{ last_seen_ts: 0 }]])
      .mockResolvedValueOnce([{ affectedRows: 1 }]);
    ...
    spy
      ...
      .mockResolvedValueOnce({ action: 'inserted', id: 5 });

    // Should not throw even though the first call rejected.
    const out = await replyActions.processVotesForRoom(db, '!r:s');
    expect(spy).toHaveBeenCalledTimes(2);
    expect(out.mediaInserted).toBe(1);
  ...
  test('rooms with no file events return zero media counters', async () => {
    matrixAdapter.readMessages.mockResolvedValueOnce([]);

    const out = await replyActions.processVotesForRoom(db, '!quiet:s');
    expect(out.mediaInserted).toBe(0);
    expect(out.mediaDuplicate).toBe(0);
    expect(out.mediaRejected).toBe(0);
  });
