...
async function escalate(db, project, trade, gap, level, narrative) {
  ...
  // Day 7+ — notify principals via Matrix
  if (daysEquivalent >= NAVEEN_NOTIFY_DAYS) {
    ...
    if (!alreadyNotified) {
      ...
      const matrixAdapter = require('../services/matrix-adapter');
      const msg = `⚠️ ${project.name} — ${trade} has been ${gap.toFixed(1)}% behind plan for ${consecutiveWeeks} weeks. Immediate attention required. ${process.env.PWA_BASE_URL}/schedule/${project.id}`;
